...
from .serializers import ProductSerializer, CartSerializer, CollectionSerializer, ReviewSerializer, CartItemSerializer, AddCartItemSerializer
...
class ProductViewSet(ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        if OrderItem.objects.filter(product_id=kwargs['id']).count() > 0:
            return Response({'error': 'Product cannot be deleted since it is associated with an order item'}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
